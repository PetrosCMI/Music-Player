...
// Show the remaining playback time in the center of the PLAY button.
function renderPlayCountdown() {
  ...
  el.playCountdown.textContent = `${m}:${s.toString().padStart(2, '0')}`;
  el.playCountdown.classList.remove('hidden');
}
...
function setLoopMode() {
  ...
  el.loopIcon.innerHTML = `<rect x="0" y="0" width="24" height="24" rx="3" fill="none" stroke="currentColor" stroke-width="2"/><text x="12" y="16" font-size="12" text-anchor="middle" fill="#ffffff" font-weight="bold">${label}</text>`;
...
function setupMediaSession() {
  ...
  try {
    navigator.mediaSession.metadata = new MediaMetadata({ title: 'Audio Player 1.0.3' });
